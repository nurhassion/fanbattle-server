  ...
  try {
    ...
    for (const p of payments) {
      if (p.status === 'Credit' && !seenPaymentIds.has(p.payment_id)) {
        ...
        // Fallback: the Payments list often doesn't include "purpose" directly for
        // Smart Page checkouts — fetch the specific payment_request to get it.
...
app.get('/dashboard', requireDashboardAuth, (req, res) => {
  ...
  const monthRows = months.map(m => `
    <tr>
      <td>${m}</td>
      <td>₹${byMonth[m].total.toLocaleString('en-IN')}</td>
      <td>₹${byMonth[m].left.toLocaleString('en-IN')}</td>
      <td>₹${byMonth[m].right.toLocaleString('en-IN')}</td>
  ...
  const dayRows = days.map(d => `
    <tr>
      <td>${d}</td>
      <td>₹${byDay[d].total.toLocaleString('en-IN')}</td>
      <td>${byDay[d].count}</td>
    </tr>`).join('');
  ...
  res.send(`
    <html>
    <head>
      <title>Fan Battle Live — Dashboard</title>
      <style>
        ...
        th{ background:#121728; }
        tr:nth-child(even){ background:#121728; }
    ...
    <body>
      ...
      <table>
        <tr><th>Month</th><th>Total</th><th>Left side</th><th>Right side</th><th>Payments</th></tr>
        ${monthRows || '<tr><td colspan="5">No records yet</td></tr>'}
      </table>

      <h2>Daily totals</h2>
      <table>
        <tr><th>Date</th><th>Total</th><th>Payments</th></tr>
        ${dayRows || '<tr><td colspan="3">No records yet</td></tr>'}
      </table>
      ...
      <table>
        <tr><th>Time</th><th>Name</th><th>Side</th><th>Amount</th><th>Email</th><th>Phone</th><th>Payment ID</th></tr>
        ${recordRows || '<tr><td colspan="7">No records yet</td></tr>'}
      </table>
    </body>
    </html>
  `);
...
app.get('/export', requireDashboardAuth, (req, res) => {
  ...
  const rows = records.map(r => {
    ...
    return [r.timestamp, r.name, r.side, r.amount, r.email, r.phone, r.purpose, r.id]
      .map(esc).join(',');
  }).join('\n');
